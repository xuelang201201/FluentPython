...
import operator
import itertools
import numbers  # <1>为了检查类型，导入 numbers 模块。
...
class Vector:
    ...
    def __mul__(self, scalar):
        # <2>如果 scalar 是 numbers.Real 某个子类的实例，用分量的乘积创建一个新 Vector 实例。
        if isinstance(scalar, numbers.Real):
            return Vector(n * scalar for n in self)
        else:  # <3>否则，返回 NotImplemented，让 Python 尝试在 scalar 操作数上调用 __rmul__ 方法。
            return NotImplemented

    def __rmul__(self, scalar):
        return self * scalar  # <4>这里，__rmul__ 方法只需执行 self * scalar，委托给 __mul__ 方法。
