...
Tests of  operator `*=` and `+=`::

    >>> v1 = Vector([1, 2, 3])
    >>> v1_alias = v1  # <1>复制一份，供后面审查 Vector([1, 2, 3]) 对象。
    >>> id(v1)  # <2>记住一开始绑定给 v1 的 Vector 实例的 ID。
    139955721315040
    >>> v1 += Vector([4, 5, 6])  # <3>增量加法运算。
    >>> v1  # <4>结果与预期相符……
    Vector([5.0, 7.0, 9.0])
    >>> id(v1)  # <5>……但是创建了新的 Vector 实例。
    139955721315136
    >>> v1_alias  # <6>审查 v1_alias，确认原来的 Vector 实例没被修改。
    Vector([1.0, 2.0, 3.0])
    >>> v1 *= 11  # <7>增量乘法运算。
    >>> v1  # <8>同样，结果与预期相符，但是创建了新的 Vector 实例。
...
class Vector:
    ...
    def __eq__(self, other):
        if isinstance(other, Vector):  # <1>如果 other 操作数是 Vector 实例（或者 Vector 子类的实例），那就像之前那样比较。
            return (len(self) == len(other) and
                    all(a == b for a, b in zip(self, other)))
        else:
            return NotImplemented  # <2> 否则，返回 NotImplemented。
