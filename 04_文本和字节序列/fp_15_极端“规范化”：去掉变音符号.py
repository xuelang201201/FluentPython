...
def dewinize(txt):
    ...
    return txt.translate(multi_map)  # dewinize 函数不影响 ASCII 或 latin1 文本，只替换 Microsoft 在 cp1252 中为
    # latin1 额外添加的字符。


def asciize(txt):
    no_marks = shave_marks_latin(dewinize(txt))  # 调用 dewinize 函数，然后去掉变音符号。
    no_marks = no_marks.replace('ß', 'ss')  # 把德语 Eszett 替换成 “ss” (这里没有使用大小写折叠，因为想保留大小写。)
...
# “Herr Voß: • ½ cup of Œtker™ caffe latte • bowl of acai.”  # 只替换了 “é” “ç” 和 “í” 三个字符。
Greek = 'Zέφupoς, Zéfiro'
print(shave_marks(Greek))
# 'Zεφupoς, Zefiro'  # “έ” 和 “é” 都被替换了。
